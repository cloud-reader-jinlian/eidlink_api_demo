...
public class Demo9803037 {

	private static String url="http://192.168.1.24:8080/appserver/facecompvouche/sync/";//通常这个url后面会拼接cid或者是appid
	...
	public static void main(String args[]) throws InvalidKeyException, NoSuchAlgorithmException,
		...
		biz_time = requestTime; // 业务处理时间
		biz_sequence_id = UUID.randomUUID().toString().replace("-", ""); // 业务流水号
		// ---------------------对公共 参数处理结束 ---------------------
		// ----------------------拼装公共参数计算sign的json开始----------------
		...
		json.element("biz_type", biz_type);
		json.element("biz_time", biz_time);
		json.element("biz_sequence_id", biz_sequence_id);
		// ----------------------拼装公共参数计算sign的json结束----------------
		// ---------------------拼装计算sign的私有参数json开始 -------------------
		json.element("reqID", reqID);
		json.element("qry_type", qry_type);
